...
disp('Souhaitez-vous mettre à la même taille des colonnes ? Recours à la fonction X(:)=[]'); 
reponse = input('Souhaitez-vous connaître la taille des échantillons ? (1=Non, 0=Oui)');
while(reponse == 0)
    variable1=input('Variable à traiter ?');
    taille=size(variable1);
    fprintf('La taille de léchantillon vaut : %d\n', taille);
    reponse = input('Avez-vous fini ? (1=Oui, 0=Non)');
end

reponse2=input('Souhaitez-vous mettre à la même taille les échantillons ? (1=Non, 0=Oui)');
while(reponse2 ==0)
    variable1=input('Variable à traiter ?');
    variable2=input('Taille souhaitée?');
    variable3=input('Taille initiale de léchantillon');
    variable1(variable2:variable3)=[]; 
    reponse2 = input('Avez-vous fini ? (1=Oui, 0=Non)');
end
